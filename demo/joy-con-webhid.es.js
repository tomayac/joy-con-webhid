  ...
  function w(d, v, f, y, x, I) {
    ...
    if (
      ...
      (O = 2 * o),
        ...
        (L = 4 * ee * s + O * G + H * te - W * Q - H + Z * p + Z * h + H * $),
        (m = 4 * p * r - K * G + 4 * h * r - V * Q),
        (a = (u * u + b * b + L * L + m * m) ** -0.5),
        (u *= a),
        (b *= a),
        (L *= a),
        (m *= a),
...
function Ce(e, t, n, c) {
  ...
  return (
    ...
    i !== 0 &&
      ((e.beta = fe * (e.beta + t.x * o) + (1 - fe) * ((n.x * ge) / i)),
      (e.gamma = fe * (e.gamma + t.y * o) + (1 - fe) * ((n.y * -ge) / i))),
    {
      alpha:
        c === 8198
...
function Ze(e) {
  ...
  return {
    ...
    y: Number.parseFloat((se(e.map(([c, l]) => l)) * t).toFixed(6)),
    z: Number.parseFloat((se(e.map(([c, l, o]) => o)) * t).toFixed(6)),
  };
}
function Re(e) {
  const t = 5e-3 * e.length,
...
class _e extends EventTarget {
  ...
  async rumble(n, c, l) {
    ...
    w === 0
      ? (g = 0)
      : w < 0.117
        ? (g = (Math.log2(w * 1e3) * 32 - 96) / (5 - w ** 2) - 1)
        : w < 0.23
          ? (g = Math.log2(w * 1e3) * 32 - 96 - 92)
          : (g = (Math.log2(w * 1e3) * 32 - 96) * 2 - 246);
        ...
        if (
          ...
          this.madgwick.update(M.x, M.y, M.z, R.x, R.y, R.z),
            (s = {
              ...
              actualOrientation: Ce(this.lastValues, M, R, l.productId),
