  ...
  const t = ({
    ...
    const T = setTimeout(() => {
      ...
      for (const [f, y] of Object.entries(r))
        if (v[Number(f) - 1] !== y)
          return;
      w.removeEventListener("inputreport", g), clearTimeout(T), setTimeout(R, 50);
    };
...
function we(e, t) {
  ...
  function w(d, v, f, y, x, I) {
    ...
    if (E = 0.5 * (-i * d - s * v - r * f), k = 0.5 * (o * d + s * f - r * v), L = 0.5 * (o * v - i * f + r * d), F = 0.5 * (o * f + i * v - s * d), !(y === 0 && x === 0 && I === 0)) {
      a = (y * y + x * x + I * I) ** -0.5;
      const G = y * a, Q = x * a, $ = I * a;
      O = 2 * o, K = 2 * i, V = 2 * s, W = 2 * r, X = 4 * o, j = 4 * i, H = 4 * s, Y = 8 * i, Z = 8 * s, ee = o * o, p = i * i, h = s * s, te = r * r, u = X * h + V * G + X * p - K * Q, b = j * te - W * G + 4 * ee * i - O * Q - j + Y * p + Y * h + j * $, B = 4 * ee * s + O * G + H * te - W * Q - H + Z * p + Z * h + H * $, m = 4 * p * r - K * G + 4 * h * r - V * Q, a = (u * u + b * b + B * B + m * m) ** -0.5, u *= a, b *= a, B *= a, m *= a, E -= c * u, k -= c * b, L -= c * B, F -= c * m;
    }
    o += E * _, i += k * _, s += L * _, r += F * _, a = (o * o + i * i + s * s + r * r) ** -0.5, o *= a, i *= a, s *= a, r *= a;
  }
  ...
  function M(d, v, f, y, x, I) {
    const a = -Math.atan2(d, Math.sqrt(v * v + f * f)), u = R(d, v, f, 1, 0, 0), b = R(1, 0, 0, u.x, u.y, u.z), B = Math.atan2(b.y, b.z), m = Math.cos(B), E = Math.sin(a), k = Math.sin(B), L = x * m - I * k, F = y * Math.cos(a) + x * k * E + I * m * E;
    return {
  ...
  function S(d, v, f, y, x, I, a, u, b, B) {
    ...
    let m, E, k, L, F, O, K, V, W, X, j, H, Y, Z, ee, p, h, te, G, Q, $, ie, be, ce, re, he, U, N, le, C, ae, A, q, ue, D;
...
const Le = {
  ...
  // biome-ignore lint/complexity/useSimpleNumberKeys:
  3: "Pro Controller"
}, fe = 0.75, Fe = 0.0125, Re = Math.PI / 2;
function Ce(e, t, n, c) {
  ...
  return e.alpha = (1 - Fe) * (e.alpha + t.z * o), i !== 0 && (e.beta = fe * (e.beta + t.x * o) + (1 - fe) * (n.x * Re / i), e.gamma = fe * (e.gamma + t.y * o) + (1 - fe) * (n.y * -Re / i)), {
...
class _e extends EventTarget {
   ...
   */
  async rumble(n, c, l) {
    const o = (g, S, d) => Math.min(Math.max(g, S), d), s = new Uint8Array(9);
    s[0] = 0;
    ...
    const T = M % 2;
    T > 0 && --M, M = M >> 1, M += 64, T > 0 && (M |= 32768), s[1] = _ & 255, s[2] = R + (_ >>> 8 & 255), s[3] = r + (M >>> 8 & 255), s[4] += M & 255;
    for (let g = 0; g < 4; g++)
      s[5 + g] = s[1 + g];
      ...
      case 48: {
        if (s = {
          ...
          const w = Xe(
            ...
            R.map((S) => S.map((d) => d.rps ?? 0))
          ), T = ge(
            R.map((S) => S.map((d) => d.dps ?? 0))
          ), g = Ze(
            w.map((S) => [
              S.x.acc ?? 0,
              S.y.acc ?? 0,
          ...
          this.madgwick.update(M.x, M.y, M.z, g.x, g.y, g.z), s = {
            ...s,
            accelerometers: w,
            gyroscopes: R,
