...
var Ae = (e, t, n) => t in e ? Ee(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : e[t] = n;
  ...
  const t = ({
    ...
    const R = setTimeout(() => {
      ...
      for (const [A, x] of Object.entries(l))
        if (E[Number(A) - 1] !== x)
          return;
      f.removeEventListener("inputreport", B), clearTimeout(R), setTimeout(M, 50);
    };
    f.addEventListener("inputreport", B), (async () => await f.sendReport(
      1,
      new Uint8Array([
        0,
        0,
...
var fe, Me;
function Be() {
  return Me || (Me = 1, fe = function(t, n) {
    n = n || {};
    const o = n.kp || 1, a = n.ki || 0;
    let i = 1 / (1e3 / t), s = n.doInitialisation !== !0, l = 2 * o;
    const u = 2 * a;
    let f = 1, M = 0, w = 0, R = 0, B = 0, L = 0, j = 0;
    function E(c, r, d, q, h, b) {
      let v, m, _, P, F, J, U;
      q !== 0 && h !== 0 && b !== 0 && (v = (q * q + h * h + b * b) ** -0.5, q *= v, h *= v, b *= v, m = M * R - f * w, _ = f * M + w * R, P = f * f - 0.5 + R * R, F = h * P - b * _, J = b * m - q * P, U = q * _ - h * m, u > 0 ? (B += u * F * i, L += u * J * i, j += u * U * i, c += B, r += L, d += j) : (B = 0, L = 0, j = 0), c += l * F, r += l * J, d += l * U), c *= 0.5 * i, r *= 0.5 * i, d *= 0.5 * i;
      const C = f, N = M, H = w;
      f += -N * c - H * r - R * d, M += C * c + H * d - R * r, w += C * r - N * d + R * c, R += C * d + N * r - H * c, v = (f * f + M * M + w * w + R * R) ** -0.5, f *= v, M *= v, w *= v, R *= v;
    }
    ...
    function x(c, r, d, q, h, b) {
      const v = -Math.atan2(c, Math.sqrt(r * r + d * d)), m = A(c, r, d, 1, 0, 0), _ = A(1, 0, 0, m.x, m.y, m.z), P = Math.atan2(_.y, _.z), F = Math.cos(P), J = Math.sin(v), U = Math.sin(P), C = h * F - b * U, N = q * Math.cos(v) + h * U * J + b * F * J;
      return {
    ...
    function S(c) {
      const r = Math.cos(c.heading * 0.5), d = Math.sin(c.heading * 0.5), q = Math.cos(c.pitch * 0.5), h = Math.sin(c.pitch * 0.5), b = Math.cos(c.roll * 0.5), v = Math.sin(c.roll * 0.5);
      return {
        w: b * q * r + v * h * d,
        x: v * q * r - b * h * d,
        y: b * h * r + v * q * d,
    ...
    function I(c, r, d, q, h, b) {
      const v = x(c, r, d, q, h, b), m = S(v), _ = (m.w * m.w + m.x * m.x + m.y * m.y + m.z * m.z) ** -0.5;
      f = m.w * _, M = m.x * _, w = m.y * _, R = m.z * _, s = !0;
    }
    function k(c, r, d, q, h, b, v, m, _, P) {
      i = P || i, s || I(q, h, b, v, m, _);
      let F, J, U, C, N, H, V, T, G, g, y, W, le, se, X, Y, ae, Z, z, ce, D, K, ee, $;
      if (v === void 0 || m === void 0 || _ === void 0 || v === 0 && m === 0 && _ === 0) {
        E(c, r, d, q, h, b);
        return;
      }
      q !== 0 && h !== 0 && b !== 0 && (F = (q * q + h * h + b * b) ** -0.5, q *= F, h *= F, b *= F, F = (v * v + m * m + _ * _) ** -0.5, v *= F, m *= F, _ *= F, J = f * f, U = f * M, C = f * w, N = f * R, H = M * M, V = M * w, T = M * R, G = w * w, g = w * R, y = R * R, W = 2 * (v * (0.5 - G - y) + m * (V - N) + _ * (T + C)), le = 2 * (v * (V + N) + m * (0.5 - H - y) + _ * (g - U)), se = Math.sqrt(W * W + le * le), X = 2 * (v * (T - C) + m * (g + U) + _ * (0.5 - H - G)), Y = T - C, ae = U + g, Z = J - 0.5 + y, z = se * (0.5 - G - y) + X * (T - C), ce = se * (V - N) + X * (U + g), D = se * (C + T) + X * (0.5 - H - G), K = h * Z - b * ae + (m * D - _ * ce), ee = b * Y - q * Z + (_ * z - v * D), $ = q * ae - h * Y + (v * ce - m * z), u > 0 ? (B += u * K * i, L += u * ee * i, j += u * $ * i, c += B, r += L, d += j) : (B = 0, L = 0, j = 0), c += l * K, r += l * ee, d += l * $), c *= 0.5 * i, r *= 0.5 * i, d *= 0.5 * i;
      const te = f, Q = M, O = w;
      f += -Q * c - O * r - R * d, M += te * c + O * d - R * r, w += te * r - Q * d + R * c, R += te * d + Q * r - O * c, F = (f * f + M * M + w * w + R * R) ** -0.5, f *= F, M *= F, w *= F, R *= F;
    }
    return {
      update: k,
      init: I,
...
var ve, Ie;
function Le() {
  return Ie || (Ie = 1, ve = function(t, n) {
    n = n || {};
    const o = 1e3 / t;
    let a = n.beta || 0.4, p = n.doInitialisation !== !0, i = 1, s = 0, l = 0, u = 0, f = 1 / o;
    function M(E, A, x, S, I, k) {
      let c, r, d, q, h, b, v, m, _, P, F, J, U, C, N, H, V, T, G, g, y, W;
      b = 0.5 * (-s * E - l * A - u * x), v = 0.5 * (i * E + l * x - u * A), m = 0.5 * (i * A - s * x + u * E), _ = 0.5 * (i * x + s * A - l * E), S === 0 && I === 0 && k === 0 || (c = (S * S + I * I + k * k) ** -0.5, S *= c, I *= c, k *= c, P = 2 * i, F = 2 * s, J = 2 * l, U = 2 * u, C = 4 * i, N = 4 * s, H = 4 * l, V = 8 * s, T = 8 * l, G = i * i, g = s * s, y = l * l, W = u * u, r = C * y + J * S + C * g - F * I, d = N * W - U * S + 4 * G * s - P * I - N + V * g + V * y + N * k, q = 4 * G * l + P * S + H * W - U * I - H + T * g + T * y + H * k, h = 4 * g * u - F * S + 4 * y * u - J * I, c = (r * r + d * d + q * q + h * h) ** -0.5, r *= c, d *= c, q *= c, h *= c, b -= a * r, v -= a * d, m -= a * q, _ -= a * h), i += b * f, s += v * f, l += m * f, u += _ * f, c = (i * i + s * s + l * l + u * u) ** -0.5, i *= c, s *= c, l *= c, u *= c;
    }
    ...
    function R(E, A, x, S, I, k) {
      const c = -Math.atan2(E, Math.sqrt(A * A + x * x)), r = w(E, A, x, 1, 0, 0), d = w(1, 0, 0, r.x, r.y, r.z), q = Math.atan2(d.y, d.z), h = Math.cos(q), b = Math.sin(c), v = Math.sin(q), m = I * h - k * v, _ = S * Math.cos(c) + I * v * b + k * h * b;
      return {
    ...
    function B(E) {
      const A = Math.cos(E.heading * 0.5), x = Math.sin(E.heading * 0.5), S = Math.cos(E.pitch * 0.5), I = Math.sin(E.pitch * 0.5), k = Math.cos(E.roll * 0.5), c = Math.sin(E.roll * 0.5);
      return {
        w: k * S * A + c * I * x,
        x: c * S * A - k * I * x,
        y: k * I * A + c * S * x,
    ...
    function L(E, A, x, S, I, k) {
      const c = R(E, A, x, S, I, k), r = B(c), d = (r.w * r.w + r.x * r.x + r.y * r.y + r.z * r.z) ** -0.5;
      i = r.w * d, s = r.x * d, l = r.y * d, u = r.z * d, p = !0;
    }
    function j(E, A, x, S, I, k, c, r, d, q) {
      f = q || f, p || L(S, I, k, c, r, d);
      let h, b, v, m, _, P, F, J, U, C, N, H, V, T, G, g, y, W, le, se, X, Y, ae, Z, z, ce, D, K, ee, $, te, Q, O, he, re;
      if (c === void 0 || r === void 0 || d === void 0 || c === 0 && r === 0 && d === 0) {
        M(E, A, x, S, I, k);
        return;
      }
      P = 0.5 * (-s * E - l * A - u * x), F = 0.5 * (i * E + l * x - u * A), J = 0.5 * (i * A - s * x + u * E), U = 0.5 * (i * x + s * A - l * E), S === 0 && I === 0 && k === 0 || (h = (S * S + I * I + k * k) ** -0.5, S *= h, I *= h, k *= h, h = (c * c + r * r + d * d) ** -0.5, c *= h, r *= h, d *= h, H = 2 * i * c, V = 2 * i * r, T = 2 * i * d, G = 2 * s * c, se = 2 * i, X = 2 * s, Y = 2 * l, ae = 2 * u, Z = 2 * i * l, z = 2 * l * u, ce = i * i, D = i * s, K = i * l, ee = i * u, $ = s * s, te = s * l, Q = s * u, O = l * l, he = l * u, re = u * u, C = c * ce - V * u + T * l + c * $ + X * r * l + X * d * u - c * O - c * re, N = H * u + r * ce - T * s + G * l - r * $ + r * O + Y * d * u - r * re, g = Math.sqrt(C * C + N * N), y = -H * l + V * s + d * ce + G * u - d * $ + Y * r * u - d * O + d * re, W = 2 * g, le = 2 * y, b = -Y * (2 * Q - Z - S) + X * (2 * D + z - I) - y * l * (g * (0.5 - O - re) + y * (Q - K) - c) + (-g * u + y * s) * (g * (te - ee) + y * (D + he) - r) + g * l * (g * (K + Q) + y * (0.5 - $ - O) - d), v = ae * (2 * Q - Z - S) + se * (2 * D + z - I) - 4 * s * (1 - 2 * $ - 2 * O - k) + y * u * (g * (0.5 - O - re) + y * (Q - K) - c) + (g * l + y * i) * (g * (te - ee) + y * (D + he) - r) + (g * u - le * s) * (g * (K + Q) + y * (0.5 - $ - O) - d), m = -se * (2 * Q - Z - S) + ae * (2 * D + z - I) - 4 * l * (1 - 2 * $ - 2 * O - k) + (-W * l - y * i) * (g * (0.5 - O - re) + y * (Q - K) - c) + (g * s + y * u) * (g * (te - ee) + y * (D + he) - r) + (g * i - le * l) * (g * (K + Q) + y * (0.5 - $ - O) - d), _ = X * (2 * Q - Z - S) + Y * (2 * D + z - I) + (-W * u + y * s) * (g * (0.5 - O - re) + y * (Q - K) - c) + (-g * i + y * l) * (g * (te - ee) + y * (D + he) - r) + g * s * (g * (K + Q) + y * (0.5 - $ - O) - d), h = (b * b + v * v + m * m + _ * _) ** -0.5, b *= h, v *= h, m *= h, _ *= h, P -= a * b, F -= a * v, J -= a * m, U -= a * _), i += P * f, s += F * f, l += J * f, u += U * f, h = (i * i + s * s + l * l + u * u) ** -0.5, i *= h, s *= h, l *= h, u *= h;
    }
    return {
      update: j,
      init: L,
...
function Ue() {
  ...
  function t(n) {
    n = n || {};
    const o = n.sampleInterval || 20, a = n.algorithm || "Madgwick";
    let p;
    if (a === "Mahony")
      p = Be();
    else if (a === "Madgwick")
      ...
      throw new Error(`AHRS(): Algorithm not valid: ${a}`);
    const i = p(o, n), s = this;
    Object.keys(i).forEach((l) => s[l] = i[l]);
  }
  return t.prototype.toVector = function() {
    const o = this.getQuaternion(), a = 2 * Math.acos(o.w), p = Math.sin(a / 2);
    return {
      angle: a,
      x: o.x / p,
      y: o.y / p,
    ...
    const o = this.getQuaternion(), a = o.w * o.w, p = o.x * o.x, i = o.y * o.y, s = o.z * o.z;
    return {
...
function de(e, t = (n) => n) {
  const n = e == null ? 0 : e.length, o = Ne(e, t);
...
const He = {
  ...
  // biome-ignore lint/complexity/useSimpleNumberKeys:
  3: "Pro Controller"
}, pe = 0.75, Oe = 0.0125, ge = Math.PI / 2;
function je(e, t, n, o) {
  const a = Date.now(), p = e.timestamp ? (a - e.timestamp) / 1e3 : 0;
  e.timestamp = a;
  const i = Math.sqrt(
    n.x ** 2 + n.y ** 2 + n.z ** 2
  );
  return e.alpha = (1 - Oe) * (e.alpha + t.z * p), i !== 0 && (e.beta = pe * (e.beta + t.x * p) + (1 - pe) * (n.x * ge / i), e.gamma = pe * (e.gamma + t.y * p) + (1 - pe) * (n.y * -ge / i)), {
    alpha: o === 8198 ? (-1 * (e.alpha * 180) / Math.PI * 430 % 90).toFixed(6) : (e.alpha * 180 / Math.PI * 430 % 360).toFixed(6),
    beta: (-1 * (e.beta * 180) / Math.PI).toFixed(6),
    gamma: o === 8198 ? (-1 * (e.gamma * 180) / Math.PI).toFixed(6) : (e.gamma * 180 / Math.PI).toFixed(6)
  };
}
function Je(e) {
  const t = 180 / Math.PI, n = e.w * e.w, o = e.x * e.x, a = e.y * e.y, p = e.z * e.z;
  return {
    alpha: (t * Math.atan2(2 * (e.x * e.y + e.z * e.w), o - a - p + n)).toFixed(6),
    beta: (t * -Math.asin(2 * (e.x * e.z - e.y * e.w))).toFixed(6),
    gamma: (t * Math.atan2(2 * (e.y * e.z + e.x * e.w), -o - a + p + n)).toFixed(6)
  };
}
function ne(e) {
...
function Qe(e) {
  const t = e.slice(15, 26), n = t.slice(0, 1)[0], o = t.slice(1, 2)[0], a = t.slice(2, 3), p = t.slice(4, 10), i = [];
  for (const u of p)
    i.push(u.toString(16));
...
class we extends EventTarget {
  ...
  constructor(n) {
    ...
    this.device = n, this.lastValues = {
      ...
      beta: 0,
      gamma: 0
    }, n.productId === 8198 ? (this.madgwick = new Re({ sampleInterval: 10, algorithm: "Madgwick" }), this.quaternion = this.madgwick.getQuaternion()) : n.productId === 8199 && (this.madgwick = new Re({ sampleInterval: 10, algorithm: "Madgwick" }), this.quaternion = this.madgwick.getQuaternion());
  }
  /**
  ...
  async enableUSBHIDJoystickReport() {
    var o;
    ((o = this.device.collections[0].outputReports) == null ? void 0 : o.find(
      (a) => a.reportId === 128
   ...
   */
  async rumble(n, o, a) {
    const p = (B, L, j) => Math.min(Math.max(B, L), j), s = new Uint8Array(9);
    s[0] = 0;
    let l = p(n, 40.875885, 626.286133), u = p(o, 81.75177, 1252.572266);
    u = (Math.round(32 * Math.log2(u * 0.1)) - 96) * 4, l = Math.round(32 * Math.log2(l * 0.1)) - 64;
    const f = p(a, 0, 1);
    let M;
    f === 0 ? M = 0 : f < 0.117 ? M = (Math.log2(f * 1e3) * 32 - 96) / (5 - f ** 2) - 1 : f < 0.23 ? M = Math.log2(f * 1e3) * 32 - 96 - 92 : M = (Math.log2(f * 1e3) * 32 - 96) * 2 - 246;
    let w = Math.round(M) * 0.5;
    const R = w % 2;
    R > 0 && --w, w = w >> 1, w += 64, R > 0 && (w |= 32768), s[1] = u & 255, s[2] = M + (u >>> 8 & 255), s[3] = l + (w >>> 8 & 255), s[4] += w & 255;
    for (let B = 0; B < 4; B++)
      s[5 + B] = s[1 + B];
   ...
   */
  async setLEDState(n) {
    const o = [0, 0, 0, 0, 0, 0, 0, 0], a = [48, n];
    await this.device.sendReport(
      1,
      new Uint8Array([...o, 0, ...a])
    );
  }
  /**
  ...
  _onInputReport({ data: n, reportId: o, device: a }) {
    ...
    ), i = Array.from(p).map((f) => f.toString(16).padStart(2, "0")).join("");
    let s = {
      ...
      case 48: {
        if (s = {
          ...
          const f = ot(
            ...
            M.map((L) => L.map((j) => j.rps ?? 0))
          ), R = ye(
            M.map((L) => L.map((j) => j.dps ?? 0))
          ), B = st(
            f.map((L) => [
              L.x.acc ?? 0,
              L.y.acc ?? 0,
          ...
          this.madgwick.update(w.x, w.y, w.z, B.x, B.y, B.z), s = {
            ...s,
            accelerometers: f,
            gyroscopes: M,
            actualAccelerometer: B,
            actualGyroscope: { dps: R, rps: w },
            actualOrientation: je(
              this.lastValues,
              w,
              B,
              a.productId
            ),
            actualOrientationQuaternion: Je(
              this.quaternion
            ),
    ...
    (l = s.deviceInfo) != null && l.type && this._receiveDeviceInfo(s.deviceInfo), (u = s.batteryLevel) != null && u.level && this._receiveBatteryLevel(s.batteryLevel), this._receiveInputEvent(s);
  }
  /**
