...
const Ee = async (e) => {
  const t =
      ...
      (w) =>
        new Promise((R, M) => {
          const T = setTimeout(() => {
              (w.removeEventListener('inputreport', g), M(new Error(_)));
            }, 5e3),
            g = (S) => {
              const d = S;
              ...
              (w.removeEventListener('inputreport', g),
                clearTimeout(T),
                setTimeout(R, 50));
            };
          (w.addEventListener('inputreport', g),
            (async () =>
              await w.sendReport(
                1,
...
function we(e, t) {
  ...
  function w(d, v, f, y, x, I) {
    let a, u, b, B, m, E, k, L, F, O, K, V, W, X, j, H, Y, Z, ee, p, h, te;
    if (
      ((E = 0.5 * (-i * d - s * v - r * f)),
      (k = 0.5 * (o * d + s * f - r * v)),
      (L = 0.5 * (o * v - i * f + r * d)),
      ...
      ((O = 2 * o),
        ...
        (te = r * r),
        (u = X * h + V * G + X * p - K * Q),
        (b = j * te - W * G + 4 * ee * i - O * Q - j + Y * p + Y * h + j * $),
        (B = 4 * ee * s + O * G + H * te - W * Q - H + Z * p + Z * h + H * $),
        (m = 4 * p * r - K * G + 4 * h * r - V * Q),
        (a = (u * u + b * b + B * B + m * m) ** -0.5),
        (u *= a),
        (b *= a),
        (B *= a),
        (m *= a),
  ...
  function M(d, v, f, y, x, I) {
    const a = -Math.atan2(d, Math.sqrt(v * v + f * f)),
      u = R(d, v, f, 1, 0, 0),
      b = R(1, 0, 0, u.x, u.y, u.z),
      B = Math.atan2(b.y, b.z),
      m = Math.cos(B),
      E = Math.sin(a),
      k = Math.sin(B),
      L = x * m - I * k,
  ...
  function S(d, v, f, y, x, I, a, u, b, B) {
    ((_ = B ?? _), l || g(y, x, I, a ?? 0, u ?? 0, b ?? 0));
    let m,
      E,
      k,
      L,
    ...
    if (
      ...
      ((H = 2 * o * oe),
        ...
        (X =
          ...
          a * D),
        (j =
          H * r + u * he - Z * i + ee * s - u * C + u * q + ie * b * r - u * D),
        (p = Math.sqrt(X * X + j * j)),
        (h =
          -H * s +
          Y * i +
        ...
        (E =
          -ie * (2 * A - ce - me) +
          $ * (2 * U + re - ve) -
          h * s * (p * (0.5 - q - D) + h * (A - N) - oe) +
          (-p * r + h * i) * (p * (ae - le) + h * (U + ue) - de) +
          p * s * (p * (N + A) + h * (0.5 - C - q) - pe)),
        (k =
          be * (2 * A - ce - me) +
          Q * (2 * U + re - ve) -
          4 * i * (1 - 2 * C - 2 * q - Ie) +
          h * r * (p * (0.5 - q - D) + h * (A - N) - oe) +
          (p * s + h * o) * (p * (ae - le) + h * (U + ue) - de) +
          (p * r - G * i) * (p * (N + A) + h * (0.5 - C - q) - pe)),
        (L =
          -Q * (2 * A - ce - me) +
          be * (2 * U + re - ve) -
          4 * s * (1 - 2 * C - 2 * q - Ie) +
          (-te * s - h * o) * (p * (0.5 - q - D) + h * (A - N) - oe) +
          (p * i + h * r) * (p * (ae - le) + h * (U + ue) - de) +
          (p * o - G * s) * (p * (N + A) + h * (0.5 - C - q) - pe)),
        (F =
          $ * (2 * A - ce - me) +
          ie * (2 * U + re - ve) +
          (-te * r + h * i) * (p * (0.5 - q - D) + h * (A - N) - oe) +
          (-p * o + h * s) * (p * (ae - le) + h * (U + ue) - de) +
          p * i * (p * (N + A) + h * (0.5 - C - q) - pe)),
        (m = (E * E + k * k + L * L + F * F) ** -0.5),
        (E *= m),
        (k *= m),
        (L *= m),
        (F *= m),
...
function Ce(e, t, n, c) {
  ...
  return (
    (e.alpha = (1 - Fe) * (e.alpha + t.z * o)),
    i !== 0 &&
      ((e.beta = fe * (e.beta + t.x * o) + (1 - fe) * ((n.x * Re) / i)),
      (e.gamma = fe * (e.gamma + t.y * o) + (1 - fe) * ((n.y * -Re) / i))),
    {
      alpha:
        c === 8198
...
function Ue(e) {
  ...
  return {
    ...
    gamma: (
      t * Math.atan2(2 * (e.y * e.z + e.x * e.w), -c - l + o + n)
    ).toFixed(6),
  };
}
function P(e) {
...
function Ze(e) {
  ...
  return {
    x: Number.parseFloat((se(e.map(([c]) => c)) * t).toFixed(6)),
    y: Number.parseFloat((se(e.map(([, c]) => c)) * t).toFixed(6)),
    z: Number.parseFloat((se(e.map(([, , c]) => c)) * t).toFixed(6)),
  };
}
function ge(e) {
  const t = 5e-3 * e.length,
...
class _e extends EventTarget {
  ...
  async enableSimpleHIDMode() {
    const l = [0, 0, 0, 0, 0, 0, 0, 0, 0, ...[3, 63]];
  ...
  async enableStandardFullMode() {
    const l = [0, 0, 0, 0, 0, 0, 0, 0, 0, ...[3, 48]];
    await this.device.sendReport(1, new Uint8Array(l));
  }
  /**
   ...
   */
  async enableIMUMode() {
    const l = [0, 0, 0, 0, 0, 0, 0, 0, 0, ...[64, 1]];
    await this.device.sendReport(1, new Uint8Array(l));
  }
  /**
  ...
  async disableIMUMode() {
    const l = [0, 0, 0, 0, 0, 0, 0, 0, 0, ...[64, 0]];
    await this.device.sendReport(1, new Uint8Array(l));
  }
  /**
  ...
  async enableVibration() {
    const l = [0, 0, 1, 64, 64, 0, 1, 64, 64, ...[72, 1]];
    await this.device.sendReport(1, new Uint8Array(l));
  }
  /**
   ...
   */
  async rumble(n, c, l) {
    const o = (g, S, d) => Math.min(Math.max(g, S), d),
      s = new Uint8Array(9);
    s[0] = 0;
    ...
    w === 0
      ? (R = 0)
      : w < 0.117
        ? (R = (Math.log2(w * 1e3) * 32 - 96) / (5 - w ** 2) - 1)
        : w < 0.23
          ? (R = Math.log2(w * 1e3) * 32 - 96 - 92)
          : (R = (Math.log2(w * 1e3) * 32 - 96) * 2 - 246);
    ...
    switch (c) {
      ...
      case 48: {
        if (
          ...
          const w = Xe(o, i),
            R = Ye(o, i),
            M = ge(R.map((S) => S.map((d) => d.rps ?? 0))),
            T = ge(R.map((S) => S.map((d) => d.dps ?? 0))),
            g = Ze(w.map((S) => [S.x.acc ?? 0, S.y.acc ?? 0, S.z.acc ?? 0]));
          (this.madgwick.update(M.x, M.y, M.z, g.x, g.y, g.z),
            (s = {
              ...s,
              accelerometers: w,
              gyroscopes: R,
              ...
              actualOrientation: Ce(this.lastValues, M, g, l.productId),
              actualOrientationQuaternion: Ue(this.quaternion),
